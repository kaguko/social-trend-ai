...
import { Flame, Sparkles, SlidersHorizontal, Radio, PlusCircle } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Cross-platform viral signals & content engine • {trendCount} active topics
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          <button
            ...
            title="API Integrations & Sources"
          >
            <SlidersHorizontal className="w-4 h-4 text-slate-400" />
            <span className="hidden sm:inline">APIs & Sources</span>
            {apiStatus?.hasGemini && (
              <span className="w-2 h-2 rounded-full bg-emerald-400" title="Gemini Connected" />
            )}
          </button>
        </div>
      </div>
